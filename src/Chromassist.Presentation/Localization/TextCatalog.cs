    ...
    private static readonly IReadOnlyDictionary<string, IReadOnlyDictionary<string, string>> Values =
        new Dictionary<string, IReadOnlyDictionary<string, string>>(StringComparer.OrdinalIgnoreCase)
        {
            ["ko"] = Create(
                ...
                ("AddContextImages", "스테이지 배경 추가"), ("RemoveContextImage", "선택 배경 제거"),
                ("ContextPickerTitle", "스테이지 배경 스크린샷 선택"),
                ("UseNeutralContext", "중립 배경 보기"),
                ("ContextPrivacyNotice", "스크린샷은 메모리에서만 처리되며 패치나 저장소에 포함되지 않습니다."),
                ("NeutralPreviewDescription", "중립 배경: 적탄 팔레트 자체의 색상 차이를 비교합니다."),
                ("StageContextPreviewDescription", "스테이지 배경: {0} 위에 실제 로컬 적탄 대표 sprite를 합성해 비교합니다."),
                ("WhyProtan", "적색 계열 색각이상에서는 일부 적·녹 계열 적탄이 서로 또는 배경과 비슷해질 수 있습니다. 색 역할 간격만 조정하며 흰 코어와 투명도는 보존하고 명도 관계는 가급적 유지합니다."),
                ("WhyDeutan", "녹색 계열 색각이상에서는 일부 녹·적 계열 적탄이 서로 또는 배경과 비슷해질 수 있습니다. 색 역할 간격만 조정하며 흰 코어와 투명도는 보존하고 명도 관계는 가급적 유지합니다."),
                ("WhyTritan", "청황색 계열 색각이상에서는 일부 청·녹·황 계열 적탄이 서로 또는 배경과 비슷해질 수 있습니다. 색 역할 간격만 조정하며 흰 코어와 투명도는 보존하고 명도 관계는 가급적 유지합니다."),
                ...
                ("VerificationPreparationDetails", "적탄 파일: {0}/{1} · 알파: {2} · 투명 픽셀: {3} · repo.js: {4} · 중성 픽셀: {5} · 변환 픽셀: {6}/{7}"),
                ("Yes", "예"), ("No", "아니요"),
            ...
            ["ja"] = Create(
                ...
                ("AddContextImages", "ステージ背景を追加"), ("RemoveContextImage", "選択背景を削除"),
                ("ContextPickerTitle", "ステージ背景のスクリーンショットを選択"),
                ("UseNeutralContext", "中立背景を表示"),
                ("ContextPrivacyNotice", "スクリーンショットはメモリ内だけで処理され、パッチやリポジトリには含まれません。"),
                ("NeutralPreviewDescription", "中立背景：敵弾パレット自体の色差を比較します。"),
                ("StageContextPreviewDescription", "ステージ背景：{0}にローカルの敵弾代表spriteを合成して比較します。"),
                ("WhyProtan", "1型色覚では一部の赤・緑系の敵弾が互いに、または背景に近く見える場合があります。色の役割間隔だけを調整し、白いコアと透明度を保持しつつ明度関係をできる限り保ちます。"),
                ("WhyDeutan", "2型色覚では一部の緑・赤系の敵弾が互いに、または背景に近く見える場合があります。色の役割間隔だけを調整し、白いコアと透明度を保持しつつ明度関係をできる限り保ちます。"),
                ("WhyTritan", "3型色覚では一部の青・緑・黄系の敵弾が互いに、または背景に近く見える場合があります。色の役割間隔だけを調整し、白いコアと透明度を保持しつつ明度関係をできる限り保ちます。"),
                ...
                ("VerificationPreparationDetails", "敵弾ファイル: {0}/{1} · アルファ: {2} · 透明ピクセル: {3} · repo.js: {4} · 中性色: {5} · 変換ピクセル: {6}/{7}"),
                ("Yes", "はい"), ("No", "いいえ"),
            ...
            ["en"] = Create(
                ...
                ("AddContextImages", "Add stage backgrounds"), ("RemoveContextImage", "Remove selected background"),
                ("ContextPickerTitle", "Select stage background screenshots"),
                ("UseNeutralContext", "Show neutral background"),
                ("ContextPrivacyNotice", "Screenshots are processed in memory only and are not added to the patch or repository."),
                ("NeutralPreviewDescription", "Neutral background: compare the enemy-projectile palette itself."),
                ("StageContextPreviewDescription", "Stage background: compare representative local enemy sprites composited over {0}."),
                ("WhyProtan", "With protan CVD, some red- and green-family enemy bullets may look similar to each other or the background. This changes only colour-role spacing, preserves white cores and alpha, and aims to retain lightness relationships."),
                ("WhyDeutan", "With deutan CVD, some green- and red-family enemy bullets may look similar to each other or the background. This changes only colour-role spacing, preserves white cores and alpha, and aims to retain lightness relationships."),
                ("WhyTritan", "With tritan CVD, some blue-, green-, and yellow-family enemy bullets may look similar to each other or the background. This changes only colour-role spacing, preserves white cores and alpha, and aims to retain lightness relationships."),
                ...
                ("VerificationPreparationDetails", "Enemy files: {0}/{1} · alpha: {2} · transparent pixels: {3} · repo.js: {4} · neutral pixels: {5} · changed pixels: {6}/{7}"),
                ("Yes", "Yes"), ("No", "No"),
