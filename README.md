...
- 적탄만 변환하고 플레이어 탄·아이템·배경·효과를 제외하는 명시적 범위 제한
- 사용자가 고른 로컬 스테이지 스크린샷 위에 적탄을 합성하는 선택적 원본/보정 비교
...
4. 세 번째 단계에서 강도를 0–100% 슬라이더로 조절합니다. 드래그 중 원본과 변경 미리보기가 연속 갱신되며, 슬라이더 트랙을 클릭하면 그 위치로 즉시 이동합니다. 원한다면 한 번의 파일 선택으로 여러 플레이 스크린샷을 불러와 각 배경 위의 적탄 원본/보정본을 비교할 수 있습니다.
5. 마지막 단계에서 `패치 준비 후 thcrap 설정 열기`를 누릅니다. 앱은 생성 파일 수, 알파 채널, 투명 픽셀, 중립색 픽셀, 변환된 불투명 픽셀 수와 `repo.js`를 자동 검사합니다.
...
맥락 비교용 스크린샷은 메모리에서만 디코딩하며 경로·이미지·합성 결과를 프로필이나 생성 패치에 저장하지 않습니다.
